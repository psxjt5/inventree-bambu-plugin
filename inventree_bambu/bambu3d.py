...
from .bambuprintercontroller import BambuPrinterController
...
class BambuLab3DPrinterDriver(ThreeDPrinterBaseDriver):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Stores BambuPrintController instances (one per printer)
        self.printerControllers: dict[int, BambuPrinterController] = {}
    ...
    def init_machine(self, machine):
        ...
        # Create a new Printer Controller for the new printer.
        if self.printerControllers.get(machine.pk) is None:
            self.printerControllers[machine.pk] = BambuPrinterController(machine)

        self.printerControllers[machine.pk].initialise()